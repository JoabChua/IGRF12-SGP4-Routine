     ...
     )
{
     ...
     *cosio2 = *cosio * (*cosio);
     ...
     ak    = pow(xke / *no, x2o3); // a1 in the STR no.3
     // is it 0.75 or 1.50 ??? check again!
     d1    = 0.75 * j2 * (3.0 * (*cosio2) - 1.0) / ((*rteosq) * (*omeosq)); // j2 = k2 in the STR no.3
     del   = d1 / (ak * ak); // del1 in the STR no.3
     adel  = ak * (1.0 - (del * del) - del * ((1.0 / 3.0) + (134.0 * del * del / 81.0))); // a0 in the STR no.3
     del   = d1/(adel * adel); // del0 in the STR no.3
     *no    = *no / (1.0 + del); // n0'' in the STR no.3
...
double ecef2lla(double r[], double lla[])
{
    ...
    latgc = latgc / pi * 180;
    lla[0] = latgd = latgd / pi * 180;
    lla[1] = lon = lon / (twopi) * 360;
    lla[2] = hellp;
    return 0;
}
